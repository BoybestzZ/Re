 ...
 * @author 75496
 */
public class A700NeutralAxisLA3S1timehistorywithfrequency {
...
//                st2.executeUpdate(createTableQuery);
            ...
            for (int i = 0; i < kasins.length; i++) {
                ...
                // Execute query and get result set
                    ResultSet rs = st.executeQuery("SELECT TESTNAME, MAX(neutralaxis) AS neutralaxis, MAX(POSITIVE) AS POSITIVE, MAX(NEGATIVE) AS NEGATIVE FROM NeuAxisLA3S1 WHERE TESTNAME = '" +testName+ "' GROUP BY TESTNAME;");
                    rs.next();
...
//            // Display the chart in a frame
            ...
            Logger.getLogger(A700NeutralAxisLA3S1timehistorywithfrequency.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
