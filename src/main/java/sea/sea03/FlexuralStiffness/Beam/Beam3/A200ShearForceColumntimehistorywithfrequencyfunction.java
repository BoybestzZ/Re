...
//                st2.executeUpdate(createTableQuery);
            ...
            for (int i = 0; i < kasins.length; i++) {
                ...
                // Execute query and get result set
                    ResultSet rs = st.executeQuery("SELECT TESTNAME, MAX(SHEARFORCE) AS SHEARFORCE, MAX(POSITIVE) AS POSITIVE, MAX(NEGATIVE) AS NEGATIVE FROM allshearforce"+columnInfo.getName()+direction+" WHERE TESTNAME = '" +testName+ "' GROUP BY TESTNAME;");
                    rs.next();
